class BaseException implements Exception {
  ...
  const BaseException(this.msg, {this.debugInfo = ''});
...
class IllegalArgumentException extends BaseException {
  const IllegalArgumentException(String msg) : super(msg);
}

/// 参数错误- 找不到指定实例, 例如通过id找不到对应的车间
class InstanceNotFoundException extends IllegalArgumentException {
  const InstanceNotFoundException(String msg) : super('找不到实例: $msg');
}
...
class InvalidStateException extends BaseException {
  const InvalidStateException(String msg) : super('状态异常: $msg');
}

/// 状态错误- 功能正在开发
class FeatDeveloping extends InvalidStateException {
  const FeatDeveloping(String msg) : super(msg);
}
